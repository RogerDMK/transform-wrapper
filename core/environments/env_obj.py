...
class DiscretePointEnv(BoundedPointEnv):
    def __init__(self, ndim: int = 4, low=-1.0, high=1.0) -> None:
        super().__init__(ndim, low, high)

    def step(self, action_axes, action_magnitude):
        assert action_axes in range(self.ndim)
        assert action_magnitude in [-1,0,1]
        action = np.zeros(self.ndim)
        action[action_axes] = action_magnitude
